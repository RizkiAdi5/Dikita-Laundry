...
@section('title', 'Buat Pesanan - LaundryDikita')
@section('page-title', 'Buat Pesanan')

@section('content')
<div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
    <form id="orderCreateForm" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" name="pickup_date" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Tanggal Selesai</label>
                <input type="date" name="delivery_date" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Diskon</label>
                <input type="number" name="discount" step="0.01" min="0" value="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Pajak</label>
                <input type="number" name="tax" step="0.01" min="0" value="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="number" name="paid_amount" step="0.01" min="0" value="0" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1">Catatan Item</label>
                <input type="text" name="item_notes" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
            </div>
            <div class="md:col-span-2">
        ...
        <div class="flex justify-end space-x-3">
            <a href="{{ route('orders.index') }}" class="px-4 py-2 text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition-colors">Batal</a>
            <button type="submit" id="orderSubmitBtn" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Simpan</button>
        </div>
    </form>
</div>
@endsection
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  form.addEventListener('submit', function(e){
    ...
    fetch("{{ route('orders.store') }}", {
      ...
      if(data.success){ window.location = "{{ route('orders.index') }}"; }
